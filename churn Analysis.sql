...
select Gender,count(*),count(*) *100/ (select count(*) from customer_data) as percentage
from Customer_data group by Gender;

select Contract,count(*) as Counts,count(*) *100/ (select count(*) from customer_data) as percentage
from Customer_data group by Contract;

select Customer_Status,count(*) as Counts,sum(Total_Revenue) as total_revenue,
sum(Total_Revenue)* 100/ (select sum(Total_Revenue) from customer_data) as percentage_of_revenue
from Customer_data group by Customer_Status;

select State,count(*) as Counts,count(*) *100/ (select count(*) from customer_data) as percentage
from Customer_data group by State
...
SELECT 
    ...
    IF(Value_Deal = "" OR Value_Deal IS NULL, 'None', Value_Deal) AS Value_Deal,
    Phone_Service,
